...
#initialization of the Tello drone and the pygame window
class FrontEnd(object):
    """ Maintains the Tello display and moves it through the keyboard keys.
        ...
        The controls are:
            ...
            - P:        Switch controllable parameter
            - v and b:  Raise or Lower controllable parameter
    """
...
#main loop of the program that reads the frames from the drone and sends the input to the drone
    def run(self):
        """
        Main loop.
...
#Input handling
    def keydown(self, key):
        ...
        if key == pygame.K_w:           # set forward velocity
            ...
            self.yaw_velocity = 0
            self.up_down_velocity = 0
        elif key == pygame.K_p:         # p switch parameters
            if self.current_parameter == 0:
                self.current_parameter = 1
            else:
                self.current_parameter = 0
        elif key == pygame.K_v:      # raise current parameter
            ...
            if self.controll_params[what] < self.controll_params_m[what] - 0.01:
                ...
                if (what == 'Color'):
                    self.reset_color()
        elif key == pygame.K_b:     # lower current parameter
            what = self.param_keys[self.current_parameter]
